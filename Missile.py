...
class House:
    def __init__(self, xpos):
        ...
        self.exploded = False
        strip = pygame.image.load("./image/home.png")
        self.images = (pygame.Surface((20, 20), pygame.SRCALPHA),
                      pygame.Surface((20, 20), pygame.SRCALPHA))
        self.images[0].blit(strip, (0, 0), Rect(0, 0, 20, 20)) #멀쩡한 이미지
        self.images[1].blit(strip, (0, 0), Rect(20, 0, 20, 20)) #부서진 이미지
...
class Missile:
    def __init__(self):
        self.max_count = 500
        self.interval = 1000
        self.pos = [0, 0] # pos[0] -> 시작점의 x좌표 , pos[1] -> 도착점의 x좌표
        self.cpos = [0, 0] # cpos[0] -> 낙하중인 x좌표
        self.firetime = 0
        self.radius = 0 #radius -> 폭발반경
        self.reload(0)

    def reload(self, time_count):
        house_x = randint(0, 12) * 60 + 20
        self.pos = (randint(0, WIDTH), house_x)
    ...
    def tick(self, time_count, shoot, houses):
        ...
        if self.radius > 0:
            ...
            self.cpos[1] = GROUND * elapsed / self.max_count
...
class Shoot:
    def __init__(self):
        self.scope = (400, 300)
        self.image = pygame.image.load("./image/scope.png")
        self.count = 0
        self.fire = False
        self.radius = 0
        self.shot_pos = (0, 0)
    ...
    def draw(self):
        ...
        if self.radius == 0 and self.count < 100:
            ...
            ypos = HEIGHT - (HEIGHT - self.shot_pos[1]) * ratio
            x_left = int((self.shot_pos[0]) * ratio)
            x_right = int((WIDTH - (WIDTH - self.shot_pos[0]) * ratio))
            pygame.draw.line(SURFACE, GREEN, (0, HEIGHT), (x_left, ypos))
            pygame.draw.line(SURFACE, GREEN, (WIDTH, HEIGHT), (x_right, ypos))

        elif self.radius > 0:
            pygame.draw.circle(SURFACE, GREEN, self.shot_pos, self.radius)
